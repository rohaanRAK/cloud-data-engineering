...
select staff_id,first_name,last_name,email from sales.staffs where (store_id = 1 AND active = 1);
...
-- Write your query below:
SELECT * FROM sales.customers ORDER BY last_name ASC OFFSET 10 ROWS FETCH NEXT 10 ROWS ONLY;
...
SELECT DISTINCT category_id, model_year FROM production.products ORDER BY category_id ASC, model_year ASC;
...
SELECT product_name, model_year, list_price FROM production.products WHERE list_price BETWEEN 300 AND 1200 AND (model_year = 2017 OR model_year = 2018) ORDER BY list_price ASC;
...
SELECT  CONCAT(c.first_name, ' ', c.last_name) AS full_name, c.email FROM sales.customers AS c WHERE c.email LIKE '%@gmail.com' ORDER BY full_name ASC;
